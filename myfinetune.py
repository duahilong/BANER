...
#        print(feat)
        ...
        for input_ids in input_ids_batch:
            ...
            pos_start = input_index + overlap_ids[0]
            pos_end = pos_start + len(entity_ids)
...
class ContrastiveTrainer(Trainer):
    def contrastive_loss(self, hidden_states, origin, pos, neg):
        ...
        # P = 2  N = 4
        # assert origin.max().item() < hidden_states.size(1), f"origin index {origin.max().item()} out of range for hidden_states dim {hidden_states.size(1)}"
        # assert pos.max().item() < hidden_states.size(1), f"pos index {pos.max().item()} out of range for hidden_states dim {hidden_states.size(1)}"
        # assert neg.max().item() < hidden_states.size(1), f"neg index {neg.max().item()} out of range for hidden_states dim {hidden_states.size(1)}"
        ...
        pos_e = torch.gather(hidden_states, 1, pos.unsqueeze(-1).expand(-1,-1,hidden_states.size(-1)))
        neg_e = torch.gather(hidden_states, 1, neg.unsqueeze(-1).expand(-1,-1,hidden_states.size(-1)))
        print(f"origin_e shape: {origin_e.shape}, pos_e shape: {pos_e.shape}, neg_e shape: {neg_e.shape}")
